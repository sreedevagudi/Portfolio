				...
				this.$drop = function(originatingPart, index, options){
					...
					function afterRevert() {
						...
						if($target){
							...
							//console.log("style" + JSON.stringify(spliced[0].type));
							// sv-on-sort callback
						...
						if(spliced != undefined){
						    if (spliced[0].type == undefined) {
						        if ($scope.checkPhoneImage == true) {
						            $scope.imageReorder("phone");
						        }
						        if ($scope.checkWatchImage == true) {
						            $scope.imageReorder("watch");
						        }
						    } else {
						        data = spliced[0].type;
						        $scope.imageReorder(data);
						    }
						    var part = $scope.part;
